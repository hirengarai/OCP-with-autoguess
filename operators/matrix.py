...
from tools.model_constraints import (
    ...
    gen_word_matrix_constraints,
    gen_word_nxor_constraints,
)
...
def gf2_multiply(
    a, b, mod_poly, degree
):  #  Multiply two elements in GF(2^m) under a given modulus polynomial
    result = 0
    while b > 0:
        if b & 1:
            result ^= a
...
def generate_gf2_elements_and_exponents(
    pri, mod_poly, degree
):  # Generate all elements of GF(2^m) and map them to their corresponding exponents (α^k).
...
def generate_binary_matrix_2(
    mod_poly, degree
):  # Construct the binary matrix for GF(2^m) based on its modulus polynomial.
...
def generate_binary_matrix_3(
    mod_poly, degree
):  # Generate the binary matrix representation for the element 3 (x + 1) in GF(2^m).
    matrix1 = generate_binary_matrix_1(degree)
    matrix2 = generate_binary_matrix_2(mod_poly, degree)
    matrix = [
        [(matrix1[i][j] + matrix2[i][j]) % 2 for j in range(len(matrix1[0]))]
...
def generate_pmr_for_mds(
    mds, mod_poly, degree
):  # Generate the Primitive Matrix Representation (PMR) for a given MDS matrix.
    sig_degree = 1 << degree
    if isinstance(mod_poly, str):
        mod_poly = int(mod_poly, 0)
    if mod_poly < sig_degree:
    ...
    elements_to_exponents, exponents_to_elements = generate_gf2_elements_and_exponents(
        pri, mod_poly, degree
    )
    if pri == 2:
        companion_matrix = matrix2
    elif pri == 3:
        companion_matrix = matrix3
    matrix_representation = {
        exp: matrix_power_mod2(companion_matrix, exp)
        for exp in range((1 << degree) - 1)
    }
    size = len(mds)
    pmr = [
        [matrix_representation[elements_to_exponents[mds[i][j]]] for j in range(size)]
        for i in range(size)
    ]
...
class Matrix(
    Operator
):  # Operator of the Matrix multiplication: appplies the matrix "mat" (stored as a list of lists) to the input vector of variables, towards the output vector of variables
    ...
    def __init__(self, name, input_vars, output_vars, mat, polynomial=None, ID=None):
        ...
        for i in mat:
            if len(i) != c:
                raise Exception(
                    str(self.__class__.__name__) + ": matrix size not consistent"
                )
        if len(input_vars) != c:
            raise Exception(
                str(self.__class__.__name__)
                + ": input vector does not match matrix size"
            )
        if len(output_vars) != r:
            raise Exception(
                str(self.__class__.__name__)
                + ": output vector does not match matrix size"
            )
    ...
    def zero_star_patterns_from_output_via_inverse(self):
        ...
        if n_rows != n_cols:
            raise ValueError(
                "M^{-1} must be square for x = M^{-1} y with same dimension."
            )
    ...
    def generate_implementation_header(self, implementation_type="python"):
        if implementation_type == "python":
            ...
            model_list.append(
                ...
                + self.name
                + "("
                + "".join(["x" + str(i) + ", " for i in range(len(self.mat[0]))])[:-2]
                + "):"
            )
            ...
            model_list.append(
                "\treturn ("
                + "".join(["y" + str(i) + ", " for i in range(len(self.mat))])[:-2]
                + ")"
            )
            return model_list
        elif implementation_type == "c":
    ...
    def generate_model(
        ...
        if model_type in ["sat", "milp"] and self.model_version in [
            ...
            if self.polynomial:  # Example: AES MDS matrix
                bin_matrix = generate_pmr_for_mds(
                    self.mat, self.polynomial, self.input_vars[0].bitsize
                )
            ...
            elif self.input_vars[0].bitsize * len(self.input_vars) == len(
                self.mat
            ):  # Example: SKINNY 64*64 binary matrix
                bin_matrix = self.mat
            else:
        ...
        # Modeling for truncated differential / linear cryptanalysis
        elif model_type in ["sat", "milp"] and self.model_version in [
            self.__class__.__name__ + "_TRUNCATEDDIFF",
            self.__class__.__name__ + "_TRUNCATEDDIFF_1",
            self.__class__.__name__ + "_TRUNCATEDLINEAR",
            self.__class__.__name__ + "_TRUNCATEDLINEAR_1",
            self.__class__.__name__ + "_TRUNCATEDDIFF_2",
            self.__class__.__name__ + "_TRUNCATEDLINEAR_2",
        ]:
            if model_type in ["milp"] and self.model_version in [
                self.__class__.__name__ + "_TRUNCATEDDIFF",
                self.__class__.__name__ + "_TRUNCATEDDIFF_1",
                self.__class__.__name__ + "_TRUNCATEDLINEAR",
                self.__class__.__name__ + "_TRUNCATEDLINEAR_1",
                ...
                if (
                    branch_num is not None
                ):  # If the branch number is provided, generate the MILP model for describing the branch number property
                    return self._generate_model_truncated_diff_linear_branch_num(
                        model_type, branch_num
                    )
            ...
            if self.model_version in [
                ...
                print(
                    f"[WARNING] The {model_type} model for differential branch number = {branch_num} is not implemented. Turn to model_version "
                    + self.model_version
                )
                ...
                print(
                    f"[WARNING] The {model_type} model for linear branch number = {branch_num} is not implemented. Turn to model_version "
                    + self.model_version
                )
            # Generate the model for describing all valid input/output patterns.
            self.model_filename = str(
                BASE_PATH
                / f"constraints_{model_type}_{self.name}_{self.model_version}_{tool_type}.txt"
            )
            self.filename_load = filename_load
            return self._generate_model_truncated_diff_linear_valid_patterns(
    ...
    def _generate_model_diff(
        self, model_type, bin_matrix
    ):  # Modeling for bit-wise differential cryptanalysis
        model_list = []
        ...
        if self.model_version in [self.__class__.__name__ + "_XORDIFF"]:
            for i in range(output_words):  # Loop over the ith output word
                for j in range(
                    bits_per_output
                ):  # Loop over the jth bit in the ith word
                    var_in = []
                    for k in range(input_words):  # Loop over the kth input word
                        for l in range(
                            bits_per_input
                        ):  # Loop over the lth bit in the kth word
                            if (
                                bin_matrix[bits_per_output * i + j][
                                    bits_per_input * k + l
                                ]
                                == 1
                            ):
                    ...
                    if model_type == "milp":
                        ...
                        d = None
                    model_list.extend(
                        gen_matrix_constraints(var_in, var_out, model_type, v_dummy=d)
                    )
            return model_list
        else:
            Exception(
    ...
    def _generate_model_linear(
        self, model_type, bin_matrix
    ):  # Modeling for bit-wise linear cryptanalysis
        model_list = []
        input_words = len(self.input_vars)
        output_words = len(self.output_vars)
        bits_per_input = self.input_vars[0].bitsize
    ...
    def _generate_model_truncated_diff_linear_branch_num(self, model_type, branch_num):
        ...
        if model_type == "milp":
            ...
            if self.model_version in [
                ...
                model_list = [
                    " + ".join(var_in + var_out) + f" - {branch_num} {var_d[0]} >= 0"
                ]
            ...
            # The second type of modeling. Reference: [1] Christina Boura, Patrick Derbez and Margot Funk. Related-Key Differential Analysis of the AES. [2] Patrick Derbez, Marie Euler, Pierre-Alain Fouque, Phuong Hoa Nguyen. Revisiting Related-Key Boomerang attacks on AES using computer-aided tool.
                ...
                model_list += [
                    " + ".join(var_in + var_out)
                    + f" - {len(var_in+var_out)} {var_d[0]} <= 0"
                ]
                model_list.append("Binary\n" + " ".join(var_in + var_out + var_d))
                return model_list
        else:
    ...
    def _generate_model_truncated_diff_linear_valid_patterns(
        self, model_type, tool_type
    ):
        input_words = len(self.input_vars)
        output_words = len(self.output_vars)
        var_in = []
        ...
        for i in range(2**input_words):
            ...
            for j in range(2**output_words):
                y = tuple(
                    "*" if b == "1" else 0 for b in format(j, f"0{output_words}b")
                )
                pattern = (x, y)
...
class GF2Linear_Trans(
    UnaryOperator
):  # Operator for the linear transformation in GF(2^n) defined by a binary matrix: y = M*x
        ...
        if implementation_type == "python":
            ...
            for i in range(n):
                ...
                if (
                    ...
                    and self.constants[i] is not None
                    and self.constants[i] != 0
                ):
    ...
    def generate_model(self, model_type="sat"):
        model_list = []
        if model_type in ["sat", "milp"] and (
            self.model_version in [self.__class__.__name__ + "_XORDIFF"]
        ):
            ...
            return model_list
        elif model_type in ["sat", "milp"] and (
            self.model_version in [self.__class__.__name__ + "_LINEAR"]
        ):
